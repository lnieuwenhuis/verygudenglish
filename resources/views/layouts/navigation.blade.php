<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
            ...
            <div class="hidden sm:flex items-center gap-6">
                <a class="hover:underline" href="{{ route('home') }}">
                    HOME
                </a>
                @if (Auth::check())
                ...
                <a class="hover:bg-gray-200 border-2 border-gray-200 py-2 px-8 -ml-3 rounded-3xl"
                    href="{{ route('studenten.resultaten') }}">
                    Resultaten
                </a>
            </div>
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden absolute bg-white w-full sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link href="{{ route('home') }}" :active="request()->routeIs('docenten')">
                HOME
            ...
            @if (Auth::check())
                <form class="hover:underline cursor-pointer" method="POST" action="#">
                    @csrf
                    <x-responsive-nav-link class="hover:text-red-500 hover:underline" type="submit" href="">
                        UITLOGGEN
                    </x-responsive-nav-link>
                </form>
            @else
                <x-responsive-nav-link class="hover:text-blue-600 hover:underline" href="#">
                    INLOGGEN
                </x-responsive-nav-link>
            @endif
